...
export default function Home({ lang }: { lang: Language }) {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#0f1012] text-[#f3f0e8]">
    ...
    <header className="absolute inset-x-0 top-0 z-30 text-white"><div className="mx-auto flex max-w-[1440px] items-center justify-between px-5 py-5 xl:px-10"><button className="flex items-center" onClick={() => scrollTo("top")} aria-label={t.home}><Mark /></button><nav className="hidden items-center gap-8 text-sm font-semibold xl:flex">{t.nav.map(([label, id]) => <button key={id} onClick={() => navigate(id)} className="nav-link">{label}</button>)}</nav><div className="hidden items-center gap-5 xl:flex"><LanguageSwitch lang={lang} /><a className="font-mono text-xs font-medium tracking-[0.1em]" href={`tel:${CONTACT.phoneHref}`} onClick={() => trackMarketingEvent("phone_click", { language: lang, placement: "desktop_header" })}>{CONTACT.phoneLabel}</a><button onClick={() => openQuote("desktop_header")} className="rounded-full bg-[#e0c070] px-5 py-3 text-xs font-extrabold uppercase tracking-[0.1em] text-[#202124] transition hover:bg-white">{t.calcShort}</button></div><div className="hidden items-center gap-3 lg:flex xl:hidden"><LanguageSwitch lang={lang} /><button onClick={() => openQuote("tablet_header")} className="rounded-full bg-[#e0c070] px-4 py-3 text-[10px] font-extrabold uppercase tracking-[0.1em] text-[#202124]">{t.calcShort}</button></div><button onClick={() => setMenuOpen(!menuOpen)} className="rounded-full border border-white/35 p-3 xl:hidden" aria-label={menuOpen ? t.closeMenu : t.openMenu}><Menu size={20} /></button></div>{menuOpen && <div className="absolute inset-x-4 top-[76px] border border-white/15 bg-[#202124]/95 p-5 shadow-2xl backdrop-blur-lg xl:hidden"><div className="mb-6 flex items-center justify-between"><span className="font-mono text-[10px] uppercase tracking-[0.15em] text-white/70">{t.language}</span><LanguageSwitch lang={lang} /></div><div className="grid gap-1">{t.nav.map(([label, id]) => <button key={id} onClick={() => navigate(id)} className="flex items-center justify-between border-b border-white/10 py-4 text-left text-xl font-bold"><span>{label}</span><MoveRight size={18} /></button>)}</div><a href={`tel:${CONTACT.phoneHref}`} onClick={() => trackMarketingEvent("phone_click", { language: lang, placement: "mobile_menu" })} className="mt-5 flex items-center gap-2 font-mono text-xs font-medium tracking-[0.1em] text-white/85"><Phone size={16} /> {CONTACT.phoneLabel}</a><button onClick={() => { setMenuOpen(false); openQuote("mobile_menu"); }} className="mt-5 w-full rounded-full bg-[#e0c070] px-5 py-4 text-sm font-extrabold uppercase tracking-[0.1em] text-[#202124]">{t.calcProject}</button></div>}</header>
    ...
    <section id="contacts" data-reveal className="bg-[#17181a] px-5 py-20 text-white sm:px-8 lg:px-10 lg:py-28"><div className="mx-auto grid max-w-[1360px] gap-14 lg:grid-cols-12"><div className="lg:col-span-5"><p className="eyebrow text-[#e0c070]">{t.contacts.kicker}</p><h2 className="section-title mt-4 text-white"><Lines lines={t.contacts.title} /></h2><div className="mt-12 grid gap-7 text-white/80"><div><p className="contact-label">{t.contacts.channel}</p><a href={`tel:${CONTACT.phoneHref}`} onClick={() => trackMarketingEvent("phone_click", { language: lang, placement: "contacts" })} className="contact-link">{CONTACT.phoneLabel}</a></div><div><p className="contact-label">{t.contacts.address}</p><p className="whitespace-pre-line text-lg leading-7">{t.contacts.schedule}</p></div><div className="flex gap-3"><a href={`https://wa.me/${CONTACT.whatsapp}`} onClick={() => trackMarketingEvent("whatsapp_click", { language: lang, placement: "contacts" })} className="contact-chip"><MessageCircle size={17} /> WhatsApp</a><button onClick={() => openQuote("contacts") } className="contact-chip"><Phone size={16} /> {t.contacts.request}</button></div></div></div><div className="relative min-h-[360px] overflow-hidden border border-white/10 bg-[#202124] lg:col-span-7"><MapView initialCenter={CONTACT.location} initialZoom={14} className="h-[420px] w-full grayscale-[.35] opacity-45" onMapReady={handleMapReady} staticFallback={isExternalStaticHost} fallback={<div className="map-fallback-content"><p className="eyebrow">{t.contacts.mapKicker}</p><h3 className="mt-3 text-3xl font-extrabold tracking-[-0.06em] text-white">{t.contacts.office}</h3><p className="mt-3 text-sm leading-6">{t.contacts.mapFallback}<br />Торетай, 43 · {t.contacts.city}</p><a href={routeHref} onClick={() => trackMarketingEvent("route_open", { language: lang, placement: "map" })} target="_blank" rel="noreferrer" className="map-route"><MapPinned size={16} /> {t.contacts.route}</a></div>} /><div className="map-grid pointer-events-none" aria-hidden="true" /><div className="pointer-events-none absolute inset-6 flex flex-col justify-between border border-[#e0c070]/35 p-5"><div className="flex items-start justify-between"><span className="font-mono text-[10px] uppercase tracking-[0.15em] text-[#e0c070]">AL / {CONTACT.coordinateLabel}</span><span className="grid h-9 w-9 place-items-center rounded-full border border-[#e0c070] bg-[#17181a] text-xs font-bold text-[#e0c070]">AL</span></div><div className="max-w-[250px] border-l border-[#e0c070] pl-4"><p className="font-mono text-[10px] uppercase tracking-[0.14em] text-[#e0c070]">{t.contacts.office}</p><p className="mt-2 text-lg font-bold leading-6 text-white">Торетай, 43<br />{t.contacts.city}</p></div></div><div className="pointer-events-none absolute bottom-5 left-5 bg-[#17181a] px-4 py-3 font-mono text-[10px] uppercase tracking-[0.12em] text-[#e0c070]">AL / {t.contacts.city}</div></div></div></section></main>
