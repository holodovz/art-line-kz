...
export default function Home({ lang }: { lang: Language }) {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#0f1012] text-[#f3f0e8]">
    ...
    <section data-reveal className="cta-sheet relative overflow-hidden bg-[#202124] px-5 py-20 text-white sm:px-8 lg:px-10 lg:py-28"><div className="absolute right-[-8vw] top-[-8vw] h-[34vw] w-[34vw] rounded-full border-[1.6vw] border-[#e0c070]/80" /><div className="cta-rule cta-rule-top" /><div className="cta-rule cta-rule-bottom" /><div className="relative mx-auto grid max-w-[1360px] gap-8 lg:grid-cols-12 lg:items-end"><div className="lg:col-span-8"><p className="eyebrow text-[#e0c070]">{t.cta.kicker} <span className="ml-4 text-white/60">{t.cta.marker}</span></p><h2 className="mt-5 text-[clamp(3.4rem,7vw,7.8rem)] font-extrabold leading-[.86] tracking-[-0.085em]"><Lines lines={t.cta.title} /></h2></div><div className="lg:col-span-4"><p className="max-w-sm text-lg leading-7 text-white/75">{t.cta.text}</p><button onClick={() => setModalOpen(true)} className="signal-button mt-8">{t.cta.action} <ArrowUpRight size={18} /></button></div></div></section>
    <section data-reveal className="section-shell bg-[#141518]"><div className="grid gap-12 lg:grid-cols-12"><div className="lg:col-span-5"><p className="eyebrow">{t.faq.kicker}</p><h2 className="section-title"><Lines lines={t.faq.title} /></h2><p className="mt-6 max-w-sm text-base leading-7 text-white/70">{t.faq.text}</p><a href={`https://wa.me/${CONTACT.whatsapp}`} className="mt-8 inline-flex items-center gap-2 border-b border-[#e0c070] pb-1 text-sm font-bold text-[#e0c070]"><MessageCircle size={17} /> {t.faq.action}</a></div><div className="lg:col-span-7">{t.faq.items.map(([question, answer], index) => <div key={question} className="border-t border-white/15"><button className="flex w-full items-center justify-between gap-6 py-6 text-left" onClick={() => setOpenFaq(openFaq === index ? null : index)} aria-expanded={openFaq === index}><span className="text-xl font-bold tracking-[-0.04em] sm:text-2xl">{question}</span><span className={`grid h-9 w-9 shrink-0 place-items-center rounded-full border border-white/25 transition ${openFaq === index ? "rotate-45 bg-[#e0c070] text-[#202124]" : ""}`}><Plus size={17} /></span></button>{openFaq === index && <p className="faq-answer">{answer}</p>}</div>)}</div></div></section>
    <section id="contacts" data-reveal className="bg-[#17181a] px-5 py-20 text-white sm:px-8 lg:px-10 lg:py-28"><div className="mx-auto grid max-w-[1360px] gap-14 lg:grid-cols-12"><div className="lg:col-span-5"><p className="eyebrow text-[#e0c070]">{t.contacts.kicker}</p><h2 className="section-title mt-4 text-white"><Lines lines={t.contacts.title} /></h2><div className="mt-12 grid gap-7 text-white/80"><div><p className="contact-label">{t.contacts.channel}</p><a href={`tel:${CONTACT.phoneHref}`} className="contact-link">{CONTACT.phoneLabel}</a></div><div><p className="contact-label">{t.contacts.address}</p><p className="whitespace-pre-line text-lg leading-7">{t.contacts.schedule}</p></div><div className="flex gap-3"><a href={`https://wa.me/${CONTACT.whatsapp}`} className="contact-chip"><MessageCircle size={17} /> WhatsApp</a><button onClick={() => setModalOpen(true)} className="contact-chip"><Phone size={16} /> {t.contacts.request}</button></div></div></div><div className="relative min-h-[360px] overflow-hidden border border-white/10 bg-[#202124] lg:col-span-7"><MapView initialCenter={CONTACT.location} initialZoom={14} className="h-[420px] w-full grayscale-[.35] opacity-45" onMapReady={handleMapReady} fallback={<div className="map-fallback-content"><p className="eyebrow">{t.contacts.mapKicker}</p><h3 className="mt-3 text-3xl font-extrabold tracking-[-0.06em] text-white">{t.contacts.office}</h3><p className="mt-3 text-sm leading-6">{t.contacts.mapFallback}<br />Торетай, 43 · {t.contacts.city}</p><a href={routeHref} target="_blank" rel="noreferrer" className="map-route"><MapPinned size={16} /> {t.contacts.route}</a></div>} /><div className="map-grid pointer-events-none" aria-hidden="true" /><div className="pointer-events-none absolute inset-6 flex flex-col justify-between border border-[#e0c070]/35 p-5"><div className="flex items-start justify-between"><span className="font-mono text-[10px] uppercase tracking-[0.15em] text-[#e0c070]">AL / {CONTACT.coordinateLabel}</span><span className="grid h-9 w-9 place-items-center rounded-full border border-[#e0c070] bg-[#17181a] text-xs font-bold text-[#e0c070]">AL</span></div><div className="max-w-[250px] border-l border-[#e0c070] pl-4"><p className="font-mono text-[10px] uppercase tracking-[0.14em] text-[#e0c070]">{t.contacts.office}</p><p className="mt-2 text-lg font-bold leading-6 text-white">Торетай, 43<br />{t.contacts.city}</p></div></div><div className="pointer-events-none absolute bottom-5 left-5 bg-[#17181a] px-4 py-3 font-mono text-[10px] uppercase tracking-[0.12em] text-[#e0c070]">AL / {t.contacts.city}</div></div></div></section></main>
