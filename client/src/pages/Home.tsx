...
export default function Home({ lang }: { lang: Language }) {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#0f1012] text-[#f3f0e8]">
    ...
    <section id="contacts" data-reveal className="bg-[#17181a] px-5 py-20 text-white sm:px-8 lg:px-10 lg:py-28"><div className="mx-auto grid max-w-[1360px] gap-14 lg:grid-cols-12"><div className="lg:col-span-5"><p className="eyebrow text-[#e0c070]">{t.contacts.kicker}</p><h2 className="section-title mt-4 text-white"><Lines lines={t.contacts.title} /></h2><div className="mt-12 grid gap-7 text-white/80"><div><p className="contact-label">{t.contacts.channel}</p><a href={`tel:${CONTACT.phoneHref}`} className="contact-link">{CONTACT.phoneLabel}</a></div><div><p className="contact-label">{t.contacts.address}</p><p className="whitespace-pre-line text-lg leading-7">{t.contacts.schedule}</p></div><div className="flex gap-3"><a href={`https://wa.me/${CONTACT.whatsapp}`} className="contact-chip"><MessageCircle size={17} /> WhatsApp</a><button onClick={() => setModalOpen(true)} className="contact-chip"><Phone size={16} /> {t.contacts.request}</button></div></div></div><div className="relative min-h-[360px] overflow-hidden border border-white/10 bg-[#202124] lg:col-span-7"><MapView initialCenter={CONTACT.location} initialZoom={14} className="h-[420px] w-full grayscale-[.35] opacity-45" onMapReady={handleMapReady} staticFallback={isExternalStaticHost} fallback={<div className="map-fallback-content"><p className="eyebrow">{t.contacts.mapKicker}</p><h3 className="mt-3 text-3xl font-extrabold tracking-[-0.06em] text-white">{t.contacts.office}</h3><p className="mt-3 text-sm leading-6">{t.contacts.mapFallback}<br />Торетай, 43 · {t.contacts.city}</p><a href={routeHref} target="_blank" rel="noreferrer" className="map-route"><MapPinned size={16} /> {t.contacts.route}</a></div>} /><div className="map-grid pointer-events-none" aria-hidden="true" /><div className="pointer-events-none absolute inset-6 flex flex-col justify-between border border-[#e0c070]/35 p-5"><div className="flex items-start justify-between"><span className="font-mono text-[10px] uppercase tracking-[0.15em] text-[#e0c070]">AL / {CONTACT.coordinateLabel}</span><span className="grid h-9 w-9 place-items-center rounded-full border border-[#e0c070] bg-[#17181a] text-xs font-bold text-[#e0c070]">AL</span></div><div className="max-w-[250px] border-l border-[#e0c070] pl-4"><p className="font-mono text-[10px] uppercase tracking-[0.14em] text-[#e0c070]">{t.contacts.office}</p><p className="mt-2 text-lg font-bold leading-6 text-white">Торетай, 43<br />{t.contacts.city}</p></div></div><div className="pointer-events-none absolute bottom-5 left-5 bg-[#17181a] px-4 py-3 font-mono text-[10px] uppercase tracking-[0.12em] text-[#e0c070]">AL / {t.contacts.city}</div></div></div></section></main>
